...
if [[ -s "$IP_FILE" ]]; then
    ...
    # Now process ASN data, filtering out 'N/A' entries
    echo "Processing ASN data to map to organizations..."
        # Now run the ASN-to-organization mapping Python script
    echo "Mapping ASNs to organizations..."
    python3 asn_mapper.py  # This will generate asn-org-mapped.csv

    echo "ASN-to-Organization mapping complete."
else
